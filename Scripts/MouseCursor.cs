    ...
    void FixedUpdate()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hit, distanceRay, mask))
        {
            if (!m_OnObject)
            {
                ...
                if (hit.transform.GetComponent<ObjectInventoryInteraction>() != null)
                {
                    imageCursor.sprite = InventorySprite;
                }
                else if(hit.transform.GetComponent<PickObjectInteraction>() != null)
                {
                    imageCursor.sprite = PickSprite;
                }
                else
                {
                    imageCursor.sprite = InteractionSprite;
                }
            }
        }
        else if (m_OnObject)
